...
def files_list(folder_path, shapefile):
    ...
    if shapefile:
        ...
        missing_files = [f for f in files if f not in files_folder]
        
        if missing_files:
            raise ValueError('The following laz tiles are missing: ', missing_files)
...
def save_file(file_name, header, x, y, z, i, index):
    indices = np.where(i == index)
    with File(file_name, mode="w", header=header) as outfile:    
        outfile.x, outfile.y, outfile.z = x[indices], y[indices], z[indices]
...
def run(lazfile, scale, dist_min, perc_factor, h_min, folder, clipp):
    ...
    if clipp:
        ...
        markers = dilation(markers, disk(int(buffer_size/scale)))
        ...
        for index in range(1, np.max(labels)):
            cloud = delayed(save_file)(folder + str(tree_index) + '.laz', header, X, Y, Z, i, index)
            clouds.append(cloud)
        ...
        markers = peak_local_max(score_array, indices=True, min_distance=int(dist_min/scale), exclude_border=True, threshold_abs= threshold_score, labels=binary_mask)
        rows, columns = markers[:,0], markers[:,1]
        lats, lons = -scale*rows + max_y - 0.5*scale, scale*columns + min_x + 0.5*scale
        print('In File: ', file,' were found trees :', len(lats), " in --- %s seconds ---" % (time.time() - start_time))
